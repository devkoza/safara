...
export default function TripdateFilter() {
  ...
  const departureDateObj = departureDateQuery ? new Date(departureDateQuery) : null;
  const returnDateObj = returnDateQuery ? new Date(returnDateQuery) : null;

  const [departureDate, setDepartureDate] = useState<Date | null>(departureDateObj);
  ...
  const handleRangeChange = (dates: [Date | null, Date | null]) => {
    ...
    setReturnDate(end);
    if (start) updateQueryparams('departureDate', format(start, 'yyyy-MM-dd'));
    if (end) updateQueryparams('returnDate', format(end, 'yyyy-MM-dd'));
  };
    ...
    <DateTime
      ...
      customInput={
        <Input
          ...
          clearable={Boolean(departureDate || returnDate)}
          endIcon={true}
